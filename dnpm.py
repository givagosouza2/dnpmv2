...
uploaded = st.file_uploader(
    "Arquivo .txt ou .csv com 4 colunas e cabeçalho (tempo, ax, ay, az)", type=["txt", "csv"])
if not uploaded:
...
if df is None or df.shape[1] < 4:
    st.error(
        "Não consegui ler 4 colunas com cabeçalho. Verifique separador e cabeçalho.")
    st.stop()

# Padroniza 4 primeiras colunas
orig_cols = list(df.columns[:4])
df = df.rename(columns=dict(zip(orig_cols, ["time", "ax", "ay", "az"])))[
    ["time", "ax", "ay", "az"]]
...
if t[-1] <= t[0]:
    st.error(
        f"Tempo inválido: t_min={t[0]:.6f} ≥ t_max={t[-1]:.6f}. Verifique a coluna de tempo.")
    st.stop()
...
fs_est = 1.0 / med_dt
st.info(
    f"Amostragem estimada: ~{fs_est:.2f} Hz | t_min={t[0]:.3f}s, t_max={t[-1]:.3f}s, N={len(t)}")
...
irreg_thresh = colp.slider(
    "Limiar irregularidade (desv/med Δt)", 0.00, 1.00, 0.10, 0.01)
...
if t_uni.size < 2:
    st.error(
        "Grade uniforme muito curta (menos de 2 pontos). Aumente duração ou diminua a fs alvo.")
    st.stop()
if t_uni[-1] < t[-1]:
...
if not is_irregular:
    ...
    st.success(
        "Pipeline: detrend → filtro 6 Hz → interpolação 100 Hz (amostragem regular).")
else:
    ...
    st.warning(f"Amostragem irregular (desv/med Δt = {irregularity:.3f} > {irreg_thresh:.3f}). "
               "Pipeline: detrend → interpolação 100 Hz → filtro 6 Hz.")

# ===== ROI =====
t0_def, t1_def = float(np.round(t_uni[0], 2)), float(np.round(t_uni[-1], 2))
t0, t1 = st.slider("Marcar ROI (segundos)", min_value=t0_def, max_value=t1_def,
                   value=(t0_def, t1_def), step=0.01)
...
# ===== Exporta processado =====
proc = pd.DataFrame({"time_s": t_uni, "ax_g": ax_p,
                    "ay_g": ay_p, "az_g": az_p})
st.download_button("⬇️ Baixar processado (CSV)",
